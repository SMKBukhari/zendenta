"use client";
import { Form } from "@/components/ui/form";
import { stepVariants } from "@/lib/constants";
import { AddCategoryFormSchema, AddComponentFormSchema } from "@/schemas";
import { useAddCategoryStore } from "@/store/useAddCategoryStore";
import { zodResolver } from "@hookform/resolvers/zod";
import { AnimatePresence, motion } from "framer-motion";
import { useRouter } from "next/navigation";
import React, { useState } from "react";
import { useForm } from "react-hook-form";
import { toast } from "sonner";
import { z } from "zod";
import CustomFormField, { FormFieldType } from "../../auth/CustomFormField";
import Dialog, { useDialog } from "../../CustomDialog";
import { Button } from "@/components/ui/button";
import axios from "axios";
import { Loader2 } from "lucide-react";
import { useAddComponentStore } from "@/store/useAddComponentStore";

const AddComponentDialog = () => {
  const { onClose } = useDialog();
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();

  const { componentName, price, setComponentName, setPrice, reset } =
    useAddComponentStore();

  const form = useForm<z.infer<typeof AddComponentFormSchema>>({
    resolver: zodResolver(AddComponentFormSchema),
    defaultValues: {
      name: componentName,
      price: price,
    },
  });

  const onSubmit = async (data: z.infer<typeof AddComponentFormSchema>) => {
    try {
      setIsLoading(true);
      const response = await axios.post("/api/components/create", data);
      toast.success(response.data.component.name, {
        description: "Component added successfully",
      });
      reset();
      router.refresh();
    } catch (error) {
      toast.error("Failed to add component");
    } finally {
      setIsLoading(false);
    }
  };
  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(onSubmit)}
        className='xl:space-y-6 space-y-4 flex flex-col h-full justify-between'
      >
        <div className='flex-1 overflow-y-auto px-4'>
          <AnimatePresence mode='wait'>
            <motion.div
              variants={stepVariants}
              initial='enter'
              animate='center'
              exit='exit'
              transition={{
                x: { type: "spring", stiffness: 300, damping: 30 },
                opacity: { duration: 0.2 },
              }}
              className='flex w-full'
            >
              <div className='space-y-5 w-full'>
                <div className='w-full space-y-6'>
                  <CustomFormField<z.infer<typeof AddComponentFormSchema>>
                    fieldType={FormFieldType.INPUT}
                    control={form.control}
                    name='name'
                    label='Component Name'
                    placeholder='Enter component name'
                    onChange={(value) => {
                      setComponentName(value as string);
                    }}
                  />

                  <CustomFormField<z.infer<typeof AddComponentFormSchema>>
                    fieldType={FormFieldType.CURRENCY_INPUT}
                    control={form.control}
                    name='price'
                    label='Component Price'
                    placeholder='Enter component price'
                    currency='$'
                    onChange={(value) => {
                      setPrice(value as number);
                    }}
                  />
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
        </div>

        <Dialog.Footer>
          <Button
            variant='outline'
            type='button'
            onClick={onClose}
            disabled={isLoading}
          >
            Cancel
          </Button>
          <Button type='submit' variant={"primary"} disabled={isLoading}>
            {isLoading ? (
              <Loader2 className='w-6 h-6 animate-spin' />
            ) : (
              "Add Component"
            )}
          </Button>
        </Dialog.Footer>
      </form>
    </Form>
  );
};

export default AddComponentDialog;
